...
static void x264_slice_idr(x264_t *h) {LOG_CALL;
    int i;
...
static int x264_slice_header_part1_read(bs_t *s,
        x264_slice_header_t *sh, x264_sps_t sps_array[32], x264_pps_t pps_array[256], int b_idr) {LOG_CALL;
    sh->i_first_mb = bs_read_ue(s, "i_first_mb");
    sh->i_type = bs_read_ue(s,"i_type");
    if (sh->i_type >= 5)
    {
        sh->i_type -= 5;
    }
    sh->i_pps_id = bs_read_ue(s,"i_pps_id");
    ...
    sh->i_frame_num = bs_read(s, sh->sps->i_log2_max_frame_num,"i_log2_max_frame_num");
    if (!sh->sps->b_frame_mbs_only)
    {
        sh->b_field_pic = bs_read1(s,"b_field_pic");
        if (sh->b_field_pic)
        {
            sh->b_bottom_field = bs_read1(s,"b_bottom_field");
        }
    }

    if (b_idr)
    {
        sh->i_idr_pic_id = bs_read_ue(s,"i_idr_pic_id");
    } else
    {
        sh->i_idr_pic_id = 0;
    }
    ...
    {
        sh->i_poc_lsb = bs_read(s, sh->sps->i_log2_max_poc_lsb,"i_log2_max_poc_lsb");
        if (sh->pps->b_pic_order && !sh->b_field_pic)
        {
            sh->i_delta_poc_bottom = bs_read_se(s,"i_delta_poc_bottom");
        }
    } else if (sh->sps->i_poc_type == 1 && !sh->sps->b_delta_pic_order_always_zero)
    {
        sh->i_delta_poc[0] = bs_read_se(s,"i_delta_poc[0]");
        if (sh->pps->b_pic_order && !sh->b_field_pic)
        {
            sh->i_delta_poc[1] = bs_read_se(s,"i_delta_poc[0]");
        }
    }

    if (sh->pps->b_redundant_pic_cnt)
    {
        sh->i_redundant_pic_cnt = bs_read_ue(s,"i_redundant_pic_cnt");
    }

    if (sh->i_type == SLICE_TYPE_B)
    {
        sh->b_direct_spatial_mv_pred = bs_read1(s,"b_direct_spatial_mv_pred");
    }

    if (sh->i_type == SLICE_TYPE_P || sh->i_type == SLICE_TYPE_SP || sh->i_type == SLICE_TYPE_B)
    {
        sh->b_num_ref_idx_override = bs_read1(s,"b_num_ref_idx_override");
        ...
        {
            sh->i_num_ref_idx_l0_active = bs_read_ue(s,"i_num_ref_idx_l0_active") + 1;
            if (sh->i_type == SLICE_TYPE_B)
            {
                sh->i_num_ref_idx_l1_active = bs_read_ue(s,"i_num_ref_idx_l1_active") + 1;
            }
        }
    }
...
static int x264_slice_header_part2_read(bs_t *s, x264_slice_header_t *sh) {LOG_CALL;
    if (sh->pps->b_cabac && sh->i_type != SLICE_TYPE_I && sh->i_type != SLICE_TYPE_SI)
    {
        sh->i_cabac_init_idc = bs_read_ue(s,"i_cabac_init_idc");
    }
    sh->i_qp_delta = bs_read_se(s,"i_qp_delta");

    if (sh->i_type == SLICE_TYPE_SI || sh->i_type == SLICE_TYPE_SP)
    {
        if (sh->i_type == SLICE_TYPE_SP)
        {
            sh->b_sp_for_swidth = bs_read1(s,"b_sp_for_swidth");
        }
        sh->i_qs_delta = bs_read_se(s,"i_qs_delta");
    }

    if (sh->pps->b_deblocking_filter_control)
    {
        sh->i_disable_deblocking_filter_idc = bs_read_ue(s,"i_disable_deblocking_filter_idc");
        if (sh->i_disable_deblocking_filter_idc != 1)
        {
            sh->i_alpha_c0_offset = bs_read_se(s,"i_alpha_c0_offset");
            sh->i_beta_offset = bs_read_se(s,"i_beta_offset");
        }
    } else
    {
...
static int x264_slice_header_ref_pic_reordering(x264_t *h, bs_t *s) {LOG_CALL;
    int b_ok;
    int i;
    ...
    {
        int b_reorder = bs_read1(s,"b_reorder");
...
static int x264_slice_header_pred_weight_table(x264_t *h, bs_t *s) {LOG_CALL;
    return -1;
}

static int x264_slice_header_dec_ref_pic_marking(x264_t *h, bs_t *s, int i_nal_type) {LOG_CALL;
    if (i_nal_type == NAL_SLICE_IDR)
    {
        int b_no_output_of_prior_pics = bs_read1(s,"b_no_output_of_prior_pics");
        int b_long_term_reference_flag = bs_read1(s,"b_long_term_reference_flag");
    ...
    {
        int b_adaptive_ref_pic_marking_mode = bs_read1(s,"b_adaptive_ref_pic_marking_mode");
 ...
 ****************************************************************************/
static int x264_slice_header_decode(x264_t *h, bs_t *s, x264_nal_t *nal) {LOG_CALL;
...
static int x264_slice_data_decode(x264_t *h, bs_t *s) {LOG_CALL;
    ...
    for (;;)
    {
        ...
        } else// cavlc
        {
            if (h->sh.i_type != SLICE_TYPE_I && h->sh.i_type != SLICE_TYPE_SI)
            {
                int i_skip = bs_read_ue(s,"i_skip");
 ...
 *
 ****************************************************************************/
int x264_nal_decode(x264_nal_t *nal, uint8_t *p_data, int i_data) {LOG_CALL;
    uint8_t *src = p_data;
 ...
 * x264_decoder_open:
 ****************************************************************************/
x264_t *x264_decoder_open(x264_param_t *param) {LOG_CALL;
    ...
    h->picture =(x264_picture_t*) x264_aligned_alloc(sizeof ( x264_picture_t));
    h->picture->i_width = 0;
    h->picture->i_height = 0;
    ...
    {
        /* max 2 step */
        h->x264_coeff_token_lookup[i] = x264_vlc_table_lookup_new(x264_coeff_token[i], 17 * 4, 8);
    }
    /* max 2 step */
...
int x264_decoder_decode(x264_t *h,
        x264_picture_t **pp_pic, x264_nal_t *nal) {LOG_CALL;
    int i_ret = 0;
    bs_t bs;
 ...
 * x264_decoder_close:
 ****************************************************************************/
void x264_decoder_close(x264_t *h) {LOG_CALL;
    int i;
