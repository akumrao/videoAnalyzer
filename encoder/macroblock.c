...
static inline void scan_zigzag_4x4full(int level[16], int16_t dct[4][4]) {LOG_CALL;
    int i;
...
static inline void scan_zigzag_4x4(int level[15], int16_t dct[4][4]) {LOG_CALL;
    int i;
...
static inline void scan_zigzag_2x2_dc(int level[4], int16_t dct[2][2]) {LOG_CALL;
    level[0] = dct[0][0];
    level[1] = dct[0][1];
    level[2] = dct[1][0];
    level[3] = dct[1][1];
...
static void quant_4x4(int16_t dct[4][4], int i_qscale, int b_intra) {LOG_CALL;
...
static void quant_4x4_dc(int16_t dct[4][4], int i_qscale) {LOG_CALL;
    const int i_qbits = 15 + i_qscale / 6;
...
static void quant_2x2_dc(int16_t dct[2][2], int i_qscale, int b_intra) {LOG_CALL;
...
static inline int array_non_zero_count(int *v, int i_count) {LOG_CALL;
    int i;
    int i_nz;
...
void x264_mb_partition_mvd(x264_macroblock_t *mb, int i_list, int i_part, int i_sub, int mvd[2]) {LOG_CALL;
    int mvp[2];
 ...
 */
static int x264_mb_decimate_score(int *dct, int i_max) {LOG_CALL;
...
void x264_mb_encode_i4x4(x264_t *h, x264_macroblock_t *mb, int idx, int i_qscale) {LOG_CALL;
...
static void x264_mb_encode_i16x16(x264_t *h, x264_macroblock_t *mb, int i_qscale) {LOG_CALL;
    x264_mb_context_t *ctx = mb->context;
...
static void x264_mb_encode_8x8(x264_t *h, x264_macroblock_t *mb, int b_inter, int i_qscale) {LOG_CALL;
    x264_mb_context_t *ctx = mb->context;
 ...
 *****************************************************************************/
void x264_macroblock_encode(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
    x264_mb_context_t *ctx = mb->context;
    int i;
...
static inline void bs_write_vlc(bs_t *s, vlc_t v, const char str[]) {LOG_CALL;
    bs_write(s, v.i_size, v.i_bits, str);
}

/****************************************************************************
 * block_residual_write_cavlc:
 ****************************************************************************/
static void block_residual_write_cavlc(x264_t *h, bs_t *s, x264_macroblock_t *mb, int i_idx, int *l, int i_count) {LOG_CALL;
    int level[16], run[16];
    ...
    {
        bs_write_vlc(s, x264_coeff_token[4][i_total * 4 + i_trailing], "vlc BLOCK_INDEX_CHROMA_DC x264_coeff_token");
    } else
    {
        ...
        bs_write_vlc(s, x264_coeff_token[ct_index[nC]][i_total * 4 + i_trailing],"vlc x264_coeff_token");
    }
    ...
    if (i_trailing > 0)
    {
        bs_write(s, i_trailing, i_sign,"i_trailing");
    }
    for (i = i_trailing; i < i_total; i++)
    {
        ...
        {
            bs_write_vlc(s, x264_level_prefix[i_level_code >> i_suffix_length],"vlc x264_level_prefix");
            if (i_suffix_length > 0)
            {
                bs_write(s, i_suffix_length, i_level_code,"i_suffix_length");
            }
        } else if (i_suffix_length == 0 && i_level_code < 30)
        {
            bs_write_vlc(s, x264_level_prefix[14],"vlc x264_level_prefix");
            bs_write(s, 4, i_level_code - 14,"i_level_code");
        ...
        {
            bs_write_vlc(s, x264_level_prefix[15],"vlc x264_level_prefix");
            ...
            bs_write(s, 12, i_level_code,"i_level_code"); /* check overflow ?? */
        }
    ...
    if (i_total < i_count)
    {
        if (i_idx == BLOCK_INDEX_CHROMA_DC)
        {
            bs_write_vlc(s, x264_total_zeros_dc[i_total - 1][i_total_zero],"vlc x264_total_zeros_dc");
        } else
        {
            bs_write_vlc(s, x264_total_zeros[i_total - 1][i_total_zero],"vlc x264_total_zeros");
        }
    }
    ...
    for (i = 0, i_zero_left = i_total_zero; i < i_total - 1; i++)
    {
        ...
        bs_write_vlc(s, x264_run_before[i_zl][run[i]],"vlc x264_run_before");
 ...
 *****************************************************************************/
void x264_macroblock_write_cavlc(x264_t *h, bs_t *s, x264_macroblock_t *mb) {LOG_CALL;
    int i_mb_i_offset;
    int i;
    ...
    {
        /* Untested */
        bs_write_ue(s, i_mb_i_offset + 25,"i_mb_i_offset");

        bs_align_0(s);
        /* Luma */
        for (i = 0; i < 16 * 16; i++)
        {
            bs_write(s, 8, h->picture->plane[0][mb->i_mb_y * 16 * h->picture->i_stride[0] + mb->i_mb_x * 16 + i], "h->picture->plane[0][");
        }
        /* Cb */
        for (i = 0; i < 8 * 8; i++)
        {
            bs_write(s, 8, h->picture->plane[1][mb->i_mb_y * 8 * h->picture->i_stride[1] + mb->i_mb_x * 8 + i], "h->picture->plane[1][");
        }
        /* Cr */
        for (i = 0; i < 8 * 8; i++)
        {
            bs_write(s, 8, h->picture->plane[2][mb->i_mb_y * 8 * h->picture->i_stride[2] + mb->i_mb_x * 8 + i], "h->picture->plane[2][");
        }
    ...
    {
        bs_write_ue(s, i_mb_i_offset + 0,"i_mb_i_offset");
        ...
        for (i = 0; i < 16; i++)
        {
            ...
            {
                bs_write1(s, 1,"b_prev_intra4x4_pred_mode"); /* b_prev_intra4x4_pred_mode */
            } else
            {
                bs_write1(s, 0,"b_prev_intra4x4_pred_mode"); /* b_prev_intra4x4_pred_mode */
                if (i_mode < i_predicted_mode)
                {
                    bs_write(s, 3, i_mode, "i_mode");
                } else
                {
                    bs_write(s, 3, i_mode - 1, "i_mode");
                }
            }
        }
        /* Prediction: chroma */
        bs_write_ue(s, mb->i_chroma_pred_mode,"i_chroma_pred_mode");
    } else if (mb->i_type == I_16x16)
    {
        bs_write_ue(s, i_mb_i_offset + 1 + mb->i_intra16x16_pred_mode +
                mb->i_cbp_chroma * 4 +
                (mb->i_cbp_luma == 0 ? 0 : 12),"i_mb_i_offset");
        /* Prediction: chroma */
        bs_write_ue(s, mb->i_chroma_pred_mode,"i_chroma_pred_mode");
    } else if (mb->i_type == P_L0)
    {
        ...
        {
            bs_write_ue(s, 0,"i_partition==D_16x16");

            if (h->sh.i_num_ref_idx_l0_active > 1)
            {
                 bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[0][0].i_ref[0],"i_num_ref_idx_l0_active");
            }
            x264_mb_predict_mv(mb, 0, 0, 0, mvp);
            bs_write_se(s, mb->partition[0][0].mv[0][0] - mvp[0],"mb->partition[0][0].mv[0][0] - mvp[0]");
            bs_write_se(s, mb->partition[0][0].mv[0][1] - mvp[1],"mb->partition[0][0].mv[0][1] - mvp[1]");
        } else if (mb->i_partition == D_16x8)
        {
            bs_write_ue(s, 1,"mb->i_partition == D_16x8");
            if (h->sh.i_num_ref_idx_l0_active > 1)
            {
                bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[0][0].i_ref[0],"i_num_ref_idx_l0_active");
                bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[0][2].i_ref[0],"i_num_ref_idx_l0_active");
            }
            ...
            bs_write_se(s, mb->partition[0][2].mv[0][0] - mvp[0],"mb->partition[0][2].mv[0][0] - mvp[0]");
            bs_write_se(s, mb->partition[0][2].mv[0][1] - mvp[1],"mb->partition[0][2].mv[0][1] - mvp[1]");
        } else if (mb->i_partition == D_8x16)
        {
            bs_write_ue(s, 2,"mb->i_partition == D_8x16");
            if (h->sh.i_num_ref_idx_l0_active > 1)
            {
                bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[0][0].i_ref[0],"i_num_ref_idx_l0_active");
                bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[2][0].i_ref[0],"i_num_ref_idx_l0_active");
            }
            ...
            x264_mb_predict_mv(mb, 0, 1, 0, mvp);
            bs_write_se(s, mb->partition[2][0].mv[0][0] - mvp[0],"mb->partition[2][0].mv[0][0] - mvp[0]");
            bs_write_se(s, mb->partition[2][0].mv[0][1] - mvp[1]," mb->partition[2][0].mv[0][1] - mvp[1]");
        }
    } else if (mb->i_type == P_8x8)
    {
        ...
        {
            bs_write_ue(s, 4, "partition");
            b_sub_ref0 = 0;
        } else
        {
            bs_write_ue(s, 3, "partition");
            b_sub_ref0 = 1;
        }
        /* sub mb type */
        for (i = 0; i < 4; i++)
        {
            ...
            {
                case D_L0_8x8:
                    bs_write_ue(s, 0,"D_L0_8x8");
                    break;
                case D_L0_8x4:
                    bs_write_ue(s, 1,"D_L0_8x4");
                    break;
                case D_L0_4x8:
                    bs_write_ue(s, 2,"D_L0_4x8");
                    break;
                case D_L0_4x4:
                    bs_write_ue(s, 3,"D_L0_4x4");
                    break;
            }
        }
        /* ref0 */
        if (h->sh.i_num_ref_idx_l0_active > 1 && b_sub_ref0)
        {
            ...
            bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[2][0].i_ref[0],"i_num_ref_idx_l0_active");
            bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[0][2].i_ref[0],"i_num_ref_idx_l0_active");
            bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, mb->partition[2][2].i_ref[0],"i_num_ref_idx_l0_active");
        }
        for (i = 0; i < 4; i++)
        {
            int i_part;
            for (i_part = 0; i_part < x264_mb_partition_count_table[mb->i_sub_partition[i]]; i_part++)
            {
                ...
                bs_write_se(s, mvx - mvp[0],"mvy - mvp[0]");
                bs_write_se(s, mvy - mvp[1],"mvy - mvp[1]");
            }
        }
    ...
    } else if (mb->i_type != B_DIRECT)
    {
        ...
        if (mb->i_partition == D_16x16)
        {
            ...
            {
                bs_write_ue(s, 3,"D_16x16 b_list0[0] && b_list1[0])");
            } else if (b_list1[0])
            {
                bs_write_ue(s, 2,"D_16x16 b_list1[0])");
            } else
            {
                bs_write_ue(s, 1,"D_16x16 not b_list0[0] && not b_list1[0])");
            }
        } else
        {
            if (mb->i_type == B_BI_BI)
            {
                bs_write_ue(s, 20 + (mb->i_partition == D_16x8 ? 0 : 1),"B_BI_BI");
            } else if (b_list0[0] && b_list1[0])
            {
                /* B_BI_LX* */
                bs_write_ue(s, 16 + (b_list0[1] ? 0 : 2) + (mb->i_partition == D_16x8 ? 0 : 1),"B_BI_LX");
            } else if (b_list0[1] && b_list1[1])
            {
                /* B_LX_BI */
                bs_write_ue(s, 12 + (b_list0[1] ? 0 : 2) + (mb->i_partition == D_16x8 ? 0 : 1),"B_LX_BI");
            } else if (b_list1[1])
            {
                /* B_LX_L1 */
                bs_write_ue(s, 6 + (b_list0[0] ? 2 : 0) + (mb->i_partition == D_16x8 ? 0 : 1),"B_LX_L1");
            } else if (b_list0[1])
            {
                /* B_LX_L0 */
                bs_write_ue(s, 4 + (b_list0[0] ? 0 : 6) + (mb->i_partition == D_16x8 ? 0 : 1),"B_LX_L0");
            }
        }
        ...
        if (h->sh.i_num_ref_idx_l0_active > 1)
        {
            for (i = 0; i < i_part; i++)
            {
                if (b_list0[i])
                {
                    ...
                    bs_write_te(s, h->sh.i_num_ref_idx_l0_active - 1, i_ref,"i_num_ref_idx_l0_active");
                }
            }
        }
        if (h->sh.i_num_ref_idx_l1_active > 1)
        {
            for (i = 0; i < i_part; i++)
            {
                if (b_list1[i])
                {
                    ...
                    bs_write_te(s, h->sh.i_num_ref_idx_l1_active - 1, i_ref,"i_num_ref_idx_l1_active");
                }
            }
        }
    ...
    if (mb->i_type == I_4x4)
    {
        bs_write_ue(s, intra4x4_cbp_to_golomb[(mb->i_cbp_chroma << 4) | mb->i_cbp_luma],"intra4x4_cbp_to_golomb");
    } else if (mb->i_type != I_16x16)
    {
        bs_write_ue(s, inter_cbp_to_golomb[(mb->i_cbp_chroma << 4) | mb->i_cbp_luma],"inter_cbp_to_golomb");
    }
    ...
    if (mb->i_type == I_16x16)
    {
        if (mb->i_mb_x > 0 || mb->i_mb_y > 0)
            bs_write_se(s, mb->i_qp - (mb - 1)->i_qp, "i_qp" );
        else
            bs_write_se(s, mb->i_qp - h->pps->i_pic_init_qp - h->sh.i_qp_delta,"i_qp");
...
static void x264_cabac_mb_type(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
...
static void x264_cabac_mb_intra4x4_pred_mode(x264_t *h, x264_macroblock_t *mb, int i_pred, int i_mode) {LOG_CALL;
    if (i_pred == i_mode)
    {
...
static void x264_cabac_mb_intra8x8_pred_mode(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
    x264_macroblock_t *mba = mb->context->mba;
    x264_macroblock_t *mbb = mb->context->mbb;
...
static void x264_cabac_mb_cbp_luma(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
    int idx;
...
static void x264_cabac_mb_cbp_chroma(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
    x264_macroblock_t *mba = mb->context->mba;
    x264_macroblock_t *mbb = mb->context->mbb;
    int i_ctxIdxInc = 0;
...
static void x264_cabac_mb_qp_delta(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
...
static int x264_cabac_mb_cbf_ctxidxinc(x264_macroblock_t *mb, int i_ctxBlockCat, int i_idx) {LOG_CALL;
    x264_mb_context_t *ctx = mb->context;
...
void x264_cabac_mb_skip(x264_t *h, x264_macroblock_t *mb, int b_skip) {LOG_CALL;
    x264_macroblock_t *mba = mb->context->mba;
    x264_macroblock_t *mbb = mb->context->mbb;
    int i_ctxIdxInc = 0;
...
static void x264_cabac_mb_ref(x264_t *h, x264_macroblock_t *mb, int i_list, int i_part) {LOG_CALL;
    x264_macroblock_t *a;
    x264_macroblock_t *b;
...
static void x264_cabac_mb_mvd(x264_t *h, int i_ctx, int i_ctx_inc, int mvd) {LOG_CALL;
    int i_abs = abs(mvd);
...
static void x264_cabac_mb_mv(x264_t *h, x264_macroblock_t *mb, int i_list, int i_part, int i_sub) {LOG_CALL;
...
static void x264_cabac_mb_sub_partition(x264_t *h, int i_sub) {LOG_CALL;
    switch (i_sub)
    {
        case D_L0_8x8:
...
static void block_residual_write_cabac(x264_t *h, x264_macroblock_t *mb, int i_ctxBlockCat, int i_idx, int *l, int i_count) {LOG_CALL;
...
void x264_macroblock_write_cabac(x264_t *h, bs_t *s, x264_macroblock_t *mb) {LOG_CALL;
    int i;
