...
void x264_mb_partition_ref_set(x264_macroblock_t *mb, int i_list, int i_part, int i_ref) {LOG_CALL;
    int x, y;
    int w, h;
    int dx, dy;
...
void x264_mb_partition_mv_set(x264_macroblock_t *mb, int i_list, int i_part, int i_sub, int mv[2]) {LOG_CALL;
    int x, y;
    int w, h;
    int dx, dy;
...
int x264_macroblock_read_cabac(x264_t *h, bs_t *s, x264_macroblock_t *mb) {LOG_CALL;
    return -1;
}

static int x264_macroblock_decode_ipcm(x264_t *h, bs_t *s, x264_macroblock_t *mb) {LOG_CALL;
    /* TODO */
    return -1;
}
...
static int bs_read_vlc(bs_t *s, x264_vlc_table_t *table) {LOG_CALL;
    int i_nb_bits;
    int i_value = 0;
    int i_bits;
    int i_index;
    ...
    myPrintf( "bits=%d, value=%d", i_bits, i_value);
...
static int block_residual_read_cavlc(x264_t *h, bs_t *s, x264_macroblock_t *mb,
        int i_idx, int *l, int i_count) {LOG_CALL;
    int i;
    int level[16], run[16];
    int i_coeff;
    ...
    if (i_idx == BLOCK_INDEX_CHROMA_DC)
    {
        ...
        if ((i_tt = bs_read_vlc(s, h->x264_coeff_token_lookup[4])) < 0)// chroma_dc_coeff_token_vlc
        {
            return -1;
        }
    ...
    for (; i < i_total; i++)
    {
        ...
        } else if (i_prefix < 14)
        {
            ...
            {
                i_level_code = (i_prefix << i_suffix_length) + bs_read(s, i_suffix_length,"i_suffix_length");
            } else
            {
        ...
        } else if (i_prefix == 14)
        {
            ...
            {
                i_level_code = i_prefix + bs_read(s, 4,"i_level_code");
            }
        } else /* if( i_prefix == 15 ) */
        {
            i_level_code = (i_prefix << i_suffix_length) + bs_read(s, 12,"i_level_code");
            if (i_suffix_length == 0)
            {
                i_level_code += 15;
            }
...
static inline void array_zero_set(int *l, int i_count) {LOG_CALL;
    int i;
...
int x264_macroblock_read_cavlc(x264_t *h, bs_t *s, x264_macroblock_t *mb) {LOG_CALL;
    int i_mb_i_offset;
    int i_mb_p_offset;
    int b_sub_ref0 = 0;
    int i_type;
    ...
    if (IS_INTRA(mb->i_type))
    {
        if (mb->i_type == I_4x4)
        {
            for (i = 0; i < 16; i++)
            {
                ...
                /*
                  MB type : I/intra coded, P/inter coded from one reference frame, B/inter coded from one or
                    two reference frames.
                    Prediction information : prediction mode(s) for an I macroblock, choice of reference frame(s)
and motion vectors for a P or B macroblock.
 Coded Block Pattern CBP : indicates which luma and chroma blocks contain non-zero
residual coefficients.
 ...
 Residual data, for blocks containing non-zero residual coefficients.
        ...
        mb->i_chroma_pred_mode = bs_read_ue(s,"i_chroma_pred_mode");
    } else if (mb->i_type == P_8x8 || mb->i_type == B_8x8)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            i_sub_partition = bs_read_ue(s,"i_sub_partition");
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            i_ref = b_sub_ref0 ? 0 : bs_read_te(s, h->sh.i_num_ref_idx_l0_active - 1,"i_ref");
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            for (i_sub = 0; i_sub < x264_mb_partition_count_table[mb->i_sub_partition[i]]; i_sub++)
            {
                ...
                mv[0] += bs_read_se(s,"mv[0]");
                mv[1] += bs_read_se(s,"mv[1]");
    ...
    } else if (mb->i_type != B_DIRECT)
    {
        ...
        for (i = 0; i < i_part; i++)
        {
            ...
            i_ref = bs_read_te(s, h->sh.i_num_ref_idx_l0_active - 1,"i_ref");
    ...
    {
        int i_cbp;
//In MPEG-2, the buffer is called VBV buffer (Video Buffer Verifier Buffer).
//In H.264, the buffer is called CPB (Coded Picture Buffer).
//The water level of the tank at certain time instance is called buffer fullness and 
//described in number of bits. The size of the tank is called VBV buffer size in MPEG-2 
//and CPB buffer size in H.264.
        i_cbp = bs_read_ue(s,"i_cbp");
        if (i_cbp >= 48)
        {
    ...
    {
        mb->i_qp = bs_read_se(s,"i_qp") + h->pps->i_pic_init_qp + h->sh.i_qp_delta;
...
static int x264_mb_pred_mode16x16_valid(x264_macroblock_t *mb, int i_mode) {LOG_CALL;
...
static int x264_mb_pred_mode8x8_valid(x264_macroblock_t *mb, int i_mode) {LOG_CALL;
    if ((mb->i_neighbour & (MB_LEFT | MB_TOP)) == (MB_LEFT | MB_TOP))
    {
        return i_mode;
    } else if ((mb->i_neighbour & MB_LEFT))
...
static int x264_mb_pred_mode4x4_valid(x264_macroblock_t *mb, int idx, int i_mode, int *pb_emu) {LOG_CALL;
    int b_a, b_b, b_c;
...
static inline void unscan_zigzag_4x4full(int16_t dct[4][4], int level[16]) {LOG_CALL;
    int i;
...
static inline void unscan_zigzag_4x4(int16_t dct[4][4], int level[15]) {LOG_CALL;
    int i;
...
static inline void unscan_zigzag_2x2_dc(int16_t dct[2][2], int level[4]) {LOG_CALL;
    dct[0][0] = level[0];
    dct[0][1] = level[1];
    dct[1][0] = level[2];
    dct[1][1] = level[3];
...
int x264_macroblock_decode(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
    ...
    if (mb->i_type == I_16x16)
    {
        ...
        dumpByte(ctx->p_fdec[0], 16,16);
       // printLong(ctx->p_fdec[0], 16,16);
...
void x264_macroblock_decode_skip(x264_t *h, x264_macroblock_t *mb) {LOG_CALL;
    int i;
    int x, y;
    int mv[2];
